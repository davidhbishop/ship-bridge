...
thermostat_digital_twin_model_identifier = "dtmi:azureiot:PhoneSensors;1"
...
interface_model_id = "dtmi:azureiot:PhoneAsADevice;2"
...
sensors_component_name = "sensors"
...
def create_max_min_report_response(sensors_name):
    """
    ...
    """
    if "PhoneSensor;1" in sensors_name and SENSORS:
        response_dict = SENSORS.create_report()
...
async def main():
    ...
    if switch == "DPS":
        ...
        registration_result = await provision_device(
            provisioning_host, id_scope, registration_id, symmetric_key, interface_model_id
        )
        ...
        if registration_result.status == "assigned":
            ...
            device_client = IoTHubDeviceClient.create_from_symmetric_key(
                ...
                product_info=interface_model_id,
            )
        else:
            raise RuntimeError(
        ...
        device_client = IoTHubDeviceClient.create_from_connection_string(
            conn_str, product_info=interface_model_id
        )
    else:
        raise RuntimeError(
    ...
    properties_thermostat1 = pnp_helper.create_reported_properties(
        sensors_component_name, maxTempSinceLastReboot=98.34
    )
    ...
    global SENSORS
    SENSORS = Thermostat(sensors_component_name, 10)
    ...
    async def send_telemetry():
        ...
        while True:
            ...
            SENSORS.record(curr_temp_ext)

            sensor_msg = {"temperature": curr_temp_ext}
            await send_telemetry_from_temp_controller(
                device_client, sensor_msg, sensors_component_name
            )
